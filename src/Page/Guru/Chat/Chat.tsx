...
const Chat: React.FC = () => {
  ...
  const fetchChatRooms = async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('Fetching guru chat rooms...');
  ...
  const fetchAvailableUsers = async () => {
    try {
      ...
      console.log('User list for new chat:', users.map((u: any) => ({ id: u.id, nama: u.nama, name: u.name, role: u.role })));
    ...
    return (
      <div className="h-full flex items-center justify-center bg-gray-900">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-white">Memuat chat guru...</span>
        </div>
      </div>
    );
  }
                      ...
                      {searchResults.map((user) => (
                        ...
                        >
                           ...
                           <div className="flex-1">
                             <h4 className="text-white font-medium">{user.nama || user.name || 'Tanpa Nama'}</h4>
