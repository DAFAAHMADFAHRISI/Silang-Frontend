...
const TambahMentorSiswa: React.FC = () => {
    ...
    const loadOptions = async () => {
      try {
        ...
        if (mentorRes.ok) {
          ...
          console.log('About to set mentorOptions with:', filtered);
          setMentorOptions(filtered);
          console.log('setMentorOptions called with length:', filtered.length);
        } else {
        ...
        console.warn('Gagal memuat opsi mentor/siswa, pastikan endpoint tersedia.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/api/mentor-siswa/create', {
        method: 'POST',
        headers: {
      ...
      const message = err instanceof Error ? err.message : 'Gagal menambahkan data mentor-siswa.';
      setError(message);
    } finally {
      setSubmitting(false);
    }
...
export default TambahMentorSiswa;
