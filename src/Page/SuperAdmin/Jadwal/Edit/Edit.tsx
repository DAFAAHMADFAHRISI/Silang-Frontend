...
const EditJadwal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        if (!id) throw new Error('ID jadwal tidak ditemukan.');
        ...
        if (!res.ok) {
          throw new Error(`Gagal mengambil detail jadwal: ${res.status} ${res.statusText}`);
        }
        ...
        setFormData({
          ...
          jam_keluar: (data.jam_keluar || '').substring(0, 5),
          location: data.location || '',
          radius_meter: String(data.radius_meter ?? ''),
        });
      } catch (err) {
        const message = err instanceof Error ? err.message : 'Gagal mengambil detail jadwal.';
        setError(message);
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/jadwal/update/${id}`, {
        method: 'PATCH',
        headers: {
      ...
      const message = err instanceof Error ? err.message : 'Gagal mengupdate jadwal.';
      setError(message);
    } finally {
      setSubmitting(false);
    }
